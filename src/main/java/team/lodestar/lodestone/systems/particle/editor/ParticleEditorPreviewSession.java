...
public class ParticleEditorPreviewSession {
    ...
    private ParticleEditorPreviewSession() {
    }

    static void start(List<ParticleEditorProject> projects) {
        stop(true);
    ...
    public static void tick(Minecraft client) {
        ...
        for (PreviewEmitter emitter : EMITTERS) {
            ParticleEditorState state = emitter.state;
    ...
    private static void spawn(ClientLevel level, Minecraft client, ParticleEditorState state) {
    ...
    private static Vec3 previewPosition(Minecraft client, ParticleEditorState state) {
    ...
    private static class PreviewEmitter {
        final ParticleEditorState state;
        int ticks;
        boolean spawned;

        PreviewEmitter(ParticleEditorState state) {
            this.state = state;
        }
    }
}
