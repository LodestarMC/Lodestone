...
class ParticleEditorScreenShapeOverlay {
    private ParticleEditorScreenShapeOverlay() {
    }

    static void render(GuiGraphics graphics, List<ParticleEditorProject> projects) {
        int index = 0;
        for (ParticleEditorProject project : projects) {
            ParticleEditorState state = project.state;
            if (project.visible && state.shapeOverlay && state.outputMode == ParticleOutputMode.SCREEN) {
                renderShape(graphics, state, ParticleEditorOverlayRenderer.overlayColor(index));
                index++;
            }
        }
    }

    private static void renderShape(GuiGraphics graphics, ParticleEditorState state, int color) {
