...
class ParticleEditorUi {
    ...
    private ParticleEditorUi() {
    }

    static void drawCentered(Font font, GuiGraphics graphics, String text, int x, int y, int width, int color) {
        graphics.drawString(font, text, x + (width - font.width(text)) / 2, y, color, false);
    }

    static void drawVerticalDots(GuiGraphics graphics, int x, int y, int width, int height, int color) {
        int dotSize = 2;
    ...
    static void drawMagnetIcon(GuiGraphics graphics, int x, int y, int width, int height, boolean active, int color) {
    ...
    static boolean inBounds(double mouseX, double mouseY, int x, int y, int w, int h) {
        return mouseX >= x && mouseX <= x + w && mouseY >= y && mouseY <= y + h;
    }

    static void renderColorSwatch(GuiGraphics graphics, String value, int x, int y, int rowHeight) {
        if (!isHexColor(value)) {
            return;
        }
    ...
    static int colorChannelAreaWidth(int rowWidth) {
        return Math.min(210, Math.max(126, rowWidth - 124));
    }

    static int shapeSizeAreaWidth(int rowWidth, int axisCount) {
        return Math.min(axisCount == 3 ? 270 : 190, Math.max(axisCount == 3 ? 174 : 116, rowWidth - 124));
    }

    static int colorChannelAt(RenderedEntry rendered, double mouseX) {
    ...
    static int shapeAxisAt(RenderedEntry rendered, double mouseX, int axisCount) {
    ...
    static String colorChannelLabel(int channel) {
    ...
    static int colorChannelText(int channel) {
        return switch (channel) {
            case 0 -> 0xFFFF8A8A;
            case 1 -> 0xFF8AFF9C;
            default -> 0xFF9CC7FF;
    ...
    static int colorChannelBackground(int channel, boolean hovered) {
    ...
    static int shapeAxisBackground(int axis, boolean hovered) {
        int alpha = hovered ? 0xA0 : 0x66;
        int rgb = switch (axis) {
            case 0 -> 0x356C9A;
            case 1 -> 0x4D8A4D;
