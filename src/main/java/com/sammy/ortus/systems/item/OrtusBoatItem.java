...
public class OrtusBoatItem extends Item {
    private static final Predicate<Entity> ENTITY_PREDICATE = EntitySelector.NO_SPECTATORS.and(Entity::isPickable);
    private final RegistryObject<EntityType<OrtusBoatEntity>> boat;

    public OrtusBoatItem(Properties properties, RegistryObject<EntityType<OrtusBoatEntity>> boat) {
        super(properties);
        this.boat = boat;
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player playerIn, InteractionHand handIn) {
        ...
        if (raytraceresult.getType() == HitResult.Type.MISS) {
            ...
            if (raytraceresult.getType() == HitResult.Type.BLOCK) {
                OrtusBoatEntity boatEntity = boat.get().create(level);
